...
print("legend: G=gold  D=dark  L=light  W=white  g=green  b=brown  .=other")
for r in 0..<rows {
    var line = ""
    for c in 0..<cols {
        let x0 = c * w / cols, x1 = (c+1) * w / cols
        let y0 = r * h / rows, y1 = (r+1) * h / rows
        var totalR = 0, totalG = 0, totalB = 0, n = 0
        ...
        else if rr < 60 && gg < 60 && bb < 60 { line += "D" }
        else if gg > rr && gg > bb && gg > 80 { line += "g" }
        else if rr > 100 && gg > 60 && bb < 80 && rr > gg { line += "b" }
